...
from .analysis_modules.feature_engineering import (
            ...
            if selected_table != "None":
                if st.sidebar.button("Fetch Data"):
                    df, err = fetch_data_postgres(
                        db_conn, selected_schema, selected_table
                    )
                    if err:
    ...
    if event_file:
        if st.session_state.get("last_event_file_id") != event_file.id:
            try:
                ...
                type_col_event = st.session_state.time_series_specs[
                    "event_event_type_col"
                ]
                ...
                if not all(col in df_event_temp.columns for col in required_event_cols):
                    st.sidebar.error(
                        f"Event data must contain specified columns: {', '.join(required_event_cols)}"
                    )
                else:
                    df_event_temp[ts_col_event] = pd.to_datetime(
                        df_event_temp[ts_col_event], errors="coerce"
                    )
                    ...
                    st.sidebar.success(
                        f"Loaded event data: {st.session_state.event_df.shape}"
                    )
    ...
    if ts_col != "None" and value_cols_to_analyze:
        ...
        if id_cols:
            if not st.session_state.data_df_original.empty:
                ...
                df_for_ids[temp_id_col_name_single_series] = (
                    df_for_ids[id_cols].astype(str).agg("-".join, axis=1)
                )
                unique_ids_list = sorted(
                    df_for_ids[temp_id_col_name_single_series].unique().tolist()
                )
        ...
        if current_selected_id not in unique_ids_display:
            current_selected_id = (
                "None" if "None" in unique_ids_display else unique_ids_display[0]
            )
        ...
        current_selected_value_col = ts_specs.get(
            "selected_value_col_for_analysis", "None"
        )
        if current_selected_value_col not in value_col_options:
            current_selected_value_col = "None"
        ...
        if st.sidebar.button(
            "Prepare Selected Time Series for Analysis",
            key="prepare_single_series_button_general",
        ):
            if (
                ts_specs["selected_id"] != "None"
                and ts_col != "None"
                and ts_specs["selected_value_col_for_analysis"] != "None"
            ):
                ...
                if id_cols and selected_entity_id_val != "DefaultTimeSeries":
                    df_orig_copy_single[temp_id_col_name_single_series] = (
                        df_orig_copy_single[id_cols].astype(str).agg("-".join, axis=1)
                    )
                    entity_series_df_single = df_orig_copy_single[
                        df_orig_copy_single[temp_id_col_name_single_series]
                        == selected_entity_id_val
                    ]

                if entity_series_df_single.empty:
                    st.sidebar.error(
                        f"No data found for ID: {selected_entity_id_val} with selected columns."
                    )
                    ts_specs["processed_series"] = None
                else:
                    try:
                        entity_series_df_single[timestamp_col_val] = pd.to_datetime(
                            entity_series_df_single[timestamp_col_val], errors="coerce"
                        )
                        st.session_state.original_selected_series_for_profiling = (
                            entity_series_df_single.set_index(timestamp_col_val)[
                                value_col_val
                            ].copy()
                        )

                        entity_series_df_single = entity_series_df_single.dropna(
                            subset=[timestamp_col_val, value_col_val]
                        )

                        if entity_series_df_single.empty:
                            st.sidebar.error(
                                f"No valid data after NA drop for {value_col_val} or {timestamp_col_val}."
                            )
                            ts_specs["processed_series"] = None
                        else:
                            entity_series_df_single = (
                                entity_series_df_single.sort_values(
                                    by=timestamp_col_val
                                )
                            )
                            processed_series_temp = (
                                entity_series_df_single.groupby(timestamp_col_val)[
                                    value_col_val
                                ]
                                .mean()
                                .rename(value_col_val)
                            )
                            ts_specs["processed_series"] = processed_series_temp
                            st.sidebar.success(
                                f"Prepared '{value_col_val}' for '{selected_entity_id_val}'. Length: {len(processed_series_temp)}"
                            )
                    except Exception as e:
                ...
                st.sidebar.warning(
                    "Please select a valid Device/Entity ID, Timestamp, and Value/Metric."
                )
                ts_specs["processed_series"] = None
    else:
        st.sidebar.info(
            "Select Timestamp and at least one Value/Metric in 'Time Series Settings' to enable single series preparation."
        )
...
def handle_population_analysis_settings():
    ...
    if (
        ts_specs.get("timestamp_col", "None") != "None"
        and ts_specs.get("selected_value_col_for_analysis", "None") != "None"
        and not st.session_state.data_df_original.empty
    ):
        compute_all_button_disabled_main = False
    else:
        st.sidebar.warning(
            "Load data and select Timestamp & a specific Value/Metric in 'Time Series Settings' to enable population feature computation."
        )

    if st.sidebar.button(
        "Compute Features for ALL Devices",
        key="compute_all_features_button_general",
        disabled=compute_all_button_disabled_main,
    ):
        ...
        try:
            parsed_acf_lags = [
                int(x.strip())
                for x in st.session_state.widget_fe_acf_lags.split(",")
                if x.strip()
            ]
            if not parsed_acf_lags:
                parsed_acf_lags = [1, 5, 10]
            st.session_state.fe_acf_lags_general = parsed_acf_lags
        except ValueError:
            st.sidebar.error("Invalid ACF Lags format. Using last valid or default.")

        try:
            parsed_rolling_windows = [
                int(x.strip())
                for x in st.session_state.widget_fe_rolling_windows.split(",")
                if x.strip()
            ]
            if not parsed_rolling_windows:
                parsed_rolling_windows = [1, 5, 10, 20]
            if any(w <= 0 for w in parsed_rolling_windows):
                st.sidebar.error(
                    "Rolling windows must be positive integers. Using last valid or default."
                )
            else:
                st.session_state.fe_rolling_windows_general = parsed_rolling_windows
        except ValueError:
            st.sidebar.error(
                "Invalid Rolling Windows format. Using last valid or default."
            )
        ...
        ts_specs_serializable["rolling_windows"] = (
            st.session_state.fe_rolling_windows_general
        )

        event_df_serializable = st.session_state.event_df.copy()
        global_top_event_types_cleaned_serializable = list(
            st.session_state.get("global_top_event_types_cleaned", [])
        )
...
def handle_save_load_app_state():
    ...
    if uploaded_settings_file_main is not None:
        try:
            ...
            success, message = apply_loaded_settings_to_session_state(
                loaded_settings_dict
            )
            if success:
                st.sidebar.success(
                    f"Settings loaded! {message} Applying and rerunning..."
                )
