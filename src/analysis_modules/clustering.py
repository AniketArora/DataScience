...
def perform_kmeans_clustering(
    feature_df: pd.DataFrame, n_clusters, random_state=42, scale_data=True, **kwargs
):
    """
    ...
    if scale_data:
        df_to_cluster, _ = scale_features(
            feature_df
        )  # Scaler object not used here but good practice
    ...
    try:
        ...
        labels = pd.Series(
            kmeans.labels_, index=df_to_cluster.index, name="kmeans_cluster_labels"
        )
...
def get_kmeans_elbow_silhouette_data(
    feature_df: pd.DataFrame, k_range=range(2, 11), scale_data=True, random_state=42
):
    """
    ...
    try:
        for k in k_range:
            ...
            if (
                k > 1
                and len(np.unique(kmeans.labels_)) > 1
                and len(np.unique(kmeans.labels_)) < len(df_to_process)
            ):
                score = silhouette_score(
                    df_to_process, kmeans.labels_, random_state=random_state
                )
...
def perform_dbscan_clustering(
    feature_df: pd.DataFrame, eps=0.5, min_samples=5, scale_data=True, **kwargs
):
    """
    ...
    try:
        ...
        labels = pd.Series(
            dbscan.labels_, index=df_to_cluster.index, name="dbscan_cluster_labels"
        )
...
if __name__ == "__main__":
    ...
    if max_k_for_test > 1:
        k_stats_df, k_stats_error = get_kmeans_elbow_silhouette_data(
            sample_features, k_range=range(2, max_k_for_test)
        )
        if k_stats_error:
    ...
    if n_samples >= 3:
        kmeans_labels, kmeans_model, kmeans_error = perform_kmeans_clustering(
            sample_features, n_clusters=3
        )
        if kmeans_error:
    ...
    if scaled_sample_features is not None and not scaled_sample_features.empty:
        dbscan_labels, dbscan_model, dbscan_error = perform_dbscan_clustering(
            scaled_sample_features, eps=1.0, min_samples=5, scale_data=False
        )  # Data already scaled
        if dbscan_error:
            print(f"Error: {dbscan_error}")
        else:
            print(
                "Cluster counts (DBSCAN, -1 is noise):\n", dbscan_labels.value_counts()
            )
    else:
...
class ClusteringAnalysisModule(AnalysisModuleInterface):
    ...
    def get_parameter_definitions(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "scale_data_kmeans": {
                "type": "checkbox",
                "default": True,
                "label": "Scale Data (K-Means)",
                "help": "Scale data before K-Means clustering.",
            },
            "eps": {
                "type": "float",
                "default": 0.5,
            ...
            "scale_data_dbscan": {
                "type": "checkbox",
                "default": True,
                "label": "Scale Data (DBSCAN)",
                "help": "Scale data before DBSCAN clustering.",
            },
    ...
    def render_parameters_ui(
        ...
        st_object.warning(
            f"{self.get_name()}: UI rendering for parameters is not fully implemented in this module stub."
        )
        ...
        updated_values["method"] = st_object.selectbox(
            ...
            index=defs["method"]["options"].index(
                current_values.get("method", defs["method"]["default"])
            ),
        ...
        if updated_values["method"] == "K-Means":
            ...
            updated_values["scale_data_kmeans"] = st_object.checkbox(
                ...
                value=current_values.get(
                    "scale_data_kmeans", defs["scale_data_kmeans"]["default"]
                ),
            ...
            updated_values["scale_data_dbscan"] = st_object.checkbox(
                ...
                value=current_values.get(
                    "scale_data_dbscan", defs["scale_data_dbscan"]["default"]
                ),
    ...
    def run_analysis(
        ...
        if data.isnull().any().any():
            return (
                None,
                "Input data contains NaN values. Please clean data before clustering.",
            )
        ...
        try:
            if method == "K-Means":
                ...
                labels, model, err = perform_kmeans_clustering(
                    data, n_clusters=n_clusters, scale_data=scale
                )
                if err:
                ...
                labels, model, err = perform_dbscan_clustering(
                    data, eps=eps, min_samples=min_samples, scale_data=scale
                )
                if err:
            ...
            logger.info(
                f"{method} clustering complete. Found {len(np.unique(results.get('labels', [])))} clusters/groups."
            )
            return results, None
    ...
    def render_results(
        ...
        if results:
            ...
            if labels is not None:
                ...
                st_object.dataframe(
                    labels.value_counts().rename("Device Count").to_frame()
                )
            if results.get("method") == "K-Means" and results.get("model"):
                try:
                    ...
                    st_object.info(
                        "Cluster centers visualization would require access to feature names used for clustering."
                    )
                except Exception as e:
