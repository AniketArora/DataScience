...
def detect_anomalies_zscore(series: pd.Series, threshold=3, window=None):
    ...
    try:
        if window:
            rolling_mean = (
                series_cleaned.rolling(window=window, center=True, min_periods=1)
                .mean()
                .reindex(series_cleaned.index)
                .bfill()
                .ffill()
            )
            rolling_std = (
        ...
        logger.error(
            "Z-score anomaly detection failed for series %s: %s",
...
def detect_anomalies_iqr(series: pd.Series, multiplier=1.5):
    ...
    try:
        ...
        logger.error(
            "IQR anomaly detection failed for series %s: %s",
            series.name if series.name else "Unnamed",
            e,
            exc_info=True,
        )
...
def detect_anomalies_isolation_forest(
    feature_df: pd.DataFrame, contamination="auto", random_state=42, **kwargs
):
    """
    ...
    try:
        ...
        labels = pd.Series(
            ...
            name="anomaly_label_iforest",
        )
        scores = pd.Series(
            model.decision_function(feature_df),
            index=feature_df.index,
            name="anomaly_score_iforest",
        )
...
def detect_anomalies_one_class_svm(
    feature_df: pd.DataFrame, nu=0.05, kernel="rbf", gamma="scale", **kwargs
):
    """
    ...
    try:
        ...
        labels = pd.Series(
            model.predict(scaled_feature_df),
            index=feature_df.index,
            name="anomaly_label_ocsvm",
        )
        scores = pd.Series(
            model.decision_function(scaled_feature_df),
            index=feature_df.index,
            name="anomaly_score_ocsvm",
        )
...
class AnomalyDetectionAnalysisModule(AnalysisModuleInterface):
    ...
    def render_parameters_ui(
        ...
        updated_values["selected_method"] = st_object.selectbox(
            ...
            index=param_defs["selected_method"]["options"].index(
                current_values.get(
                    "selected_method", param_defs["selected_method"]["default"]
                )
            ),
        ...
        if updated_values["selected_method"] == "Isolation Forest":
            ...
            updated_values["iforest_contamination"] = st_object.slider(
                ...
                value=current_values.get(
                    "iforest_contamination",
                    param_defs["iforest_contamination"]["default"],
                ),
            ...
            updated_values["ocsvm_kernel"] = st_object.selectbox(
                ...
                index=param_defs["ocsvm_kernel"]["options"].index(
                    current_values.get(
                        "ocsvm_kernel", param_defs["ocsvm_kernel"]["default"]
                    )
                ),
            ...
            updated_values["ocsvm_gamma"] = st_object.text_input(
                ...
                value=str(
                    current_values.get(
                        "ocsvm_gamma", param_defs["ocsvm_gamma"]["default"]
                    )
                ),  # Ensure value is str for text_input
        ...
        for param_name, definition in param_defs.items():
            if param_name not in updated_values:
                updated_values[param_name] = current_values.get(
                    param_name, definition["default"]
                )
    ...
    def run_analysis(
        ...
        if not all(
            dtype.kind in "biufc" for dtype in data_df.dtypes
        ):  # Check if all columns are numeric
            return (
                None,
                "All columns in input data must be numeric for these anomaly detection methods.",
            )
        if data_df.isnull().values.any():  # Check for any NaNs
            return (
                None,
                "Input data contains NaN values. Please ensure data is cleaned (e.g., features_df_cleaned.dropna()).",
            )
        ...
        try:
            if method == "Isolation Forest":
                ...
                labels, scores, error_msg = detect_anomalies_isolation_forest(
                    data_df, contamination=contamination
                )
                ...
                labels, scores, error_msg = detect_anomalies_one_class_svm(
                    data_df, nu=nu, kernel=kernel, gamma=gamma_val
                )
            else:
            ...
            if labels is not None and scores is not None:
                results_payload.update(
                    {"labels": labels, "scores": scores, "status": "detection_done"}
                )
                return results_payload, None
            else:  # Should be caught by error_msg from detection functions, but as a safeguard
                return (
                    None,
                    f"Anomaly detection for {method} failed to return labels or scores.",
                )
    ...
    def render_results(
        ...
        labels: pd.Series | None = results.get("labels")
        scores: pd.Series | None = results.get("scores")
        ...
        if labels is not None and scores is not None:
            ...
            st_object.write(
                f"Found **{num_anomalies}** potential anomalies out of {len(labels)} devices."
            )

            res_df = pd.DataFrame({"label": labels, "score": scores}).sort_values(
                by="score"
            )
            st_object.write(
                "Top potentially anomalous devices (lower scores are more anomalous):"
            )
            st_object.dataframe(res_df[res_df["label"] == -1].head())

            st_object.write(
                "Top potentially normal devices (higher scores are less anomalous):"
            )
            st_object.dataframe(
                res_df[res_df["label"] == 1]
            ...
            try:
                ...
                if len(scores_to_plot) > 1000:
                    scores_to_plot = scores_to_plot.iloc[
                        np.linspace(0, len(scores_to_plot) - 1, 1000, dtype=int)
                    ]  # Sample
...
if __name__ == "__main__":
    ...
    anomalies_z, z_scores, error_z = detect_anomalies_zscore(
        sample_series, threshold=2.5
    )
    if error_z:
    ...
    anomalies_z_roll, z_scores_roll, error_z_roll = detect_anomalies_zscore(
        sample_series, threshold=2.0, window=10
    )
    if error_z_roll:
        print(f"Error: {error_z_roll}")
    else:
        print(
            "Anomalies detected at (rolling):",
            anomalies_z_roll[anomalies_z_roll].index.tolist(),
        )

    print("\n--- IQR Anomalies ---")
    anomalies_iqr, bounds, error_iqr = detect_anomalies_iqr(
        sample_series, multiplier=1.5
    )
    if error_iqr:
    ...
    if err_z_const:
        print(f"Z-score on constant series error: {err_z_const}")
    else:
        print(
            f"Z-score on constant series anomalies: {an_z_const.sum()}, Z-scores NaN: {zs_const.isna().all()}"
        )

    an_z_const_w, zs_const_w, err_z_const_w = detect_anomalies_zscore(
        constant_series, window=5
    )
    if err_z_const_w:
        print(f"Z-score (window) on constant series error: {err_z_const_w}")
    else:
        print(
            f"Z-score (window) on constant series anomalies: {an_z_const_w.sum()}, Z-scores NaN: {zs_const_w.isna().all()}"
        )
    ...
    X_outliers = pd.DataFrame(
        rng.uniform(low=-4, high=4, size=(10, n_features)), columns=X_train.columns
    )
    ...
    print("\n--- Isolation Forest ---")
    if_labels, if_scores, if_error = detect_anomalies_isolation_forest(
        sample_feature_df, contamination=0.15
    )
    if if_error:
    ...
    print("\n--- One-Class SVM ---")
    ocsvm_labels, ocsvm_scores, ocsvm_error = detect_anomalies_one_class_svm(
        sample_feature_df, nu=0.15
    )
    if ocsvm_error:
