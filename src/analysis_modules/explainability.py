...
)  # plot_tree is not used in this module directly
...
def get_cluster_feature_summary(feature_df: pd.DataFrame, cluster_labels: pd.Series):
    ...
    """
    if not isinstance(feature_df, pd.DataFrame) or not isinstance(
...
@st.cache_data
def get_feature_importance_for_clusters_anova(
    feature_df: pd.DataFrame, cluster_labels: pd.Series
):
    """
    ...
    for feature_name in numeric_feature_df.columns:
        try:
            groups = [
                numeric_feature_df[feature_name][cluster_labels_cleaned == c]
                for c in cluster_labels_cleaned.unique()
            ]
            groups = [
                g for g in groups if len(g.dropna()) > 1 and g.dropna().var() > 1e-6
            ]  # Ensure variance > 0 and enough samples
            if len(groups) < 2:
            ...
            logger.warning(
                "ANOVA calculation failed for feature '%s': %s", feature_name, e_anova
            )
    ...
    importance_df = pd.DataFrame(
        {"Feature": features, "F-Value": f_values, "P-Value": p_values}
    )
    return importance_df.sort_values(by="F-Value", ascending=False).reset_index(
        drop=True
    ), None


@st.cache_data
def compare_anomalous_vs_normal_features(
    feature_df: pd.DataFrame, anomaly_labels: pd.Series, anomalous_label_val=-1
):
    """
    ...
    try:
        ...
        if not normal_mask.any():
            ...
            logger.warning(msg)
            anomalous_features_mean = (
                numeric_feature_df[anomalous_mask].mean().rename("Anomalous_Mean")
            )
        ...
        if not anomalous_mask.any():
            ...
            normal_features_mean = (
                numeric_feature_df[normal_mask].mean().rename("Normal_Mean")
            )
        ...
        comparison_df = pd.concat(
            [normal_features_mean, anomalous_features_mean], axis=1
        )
        comparison_df["Difference (Anomalous - Normal)"] = (
            comparison_df["Anomalous_Mean"] - comparison_df["Normal_Mean"]
        )
        comparison_df["Relative_Difference (%)"] = (
            comparison_df["Difference (Anomalous - Normal)"]
            / comparison_df["Normal_Mean"].abs().replace(0, np.nan)
        ) * 100
        comparison_df["Relative_Difference (%)"] = comparison_df[
            "Relative_Difference (%)"
        ].replace([np.inf, -np.inf], np.nan)
        return comparison_df.sort_values(
            by="Difference (Anomalous - Normal)", key=abs, ascending=False
        ), None
    except Exception as e:
        logger.error(
            "Error comparing anomalous vs normal features: %s", e, exc_info=True
        )
...
def explain_anomalies_with_surrogate_model(
    ...
    if anomaly_labels.nunique() < 2:
        ...
        return None, None, None, msg
    X = feature_df
    y = anomaly_labels.copy()
    try:
        surrogate_tree = DecisionTreeClassifier(
            max_depth=max_depth, random_state=random_state, class_weight="balanced"
        )
        report_dict = None
        if (
            test_size > 0
            and test_size < 1
            and len(X) * test_size >= 2
            and y.nunique() >= 2
        ):
            try:
                ...
                ValueError
            ) as ve:  # Stratify can fail if one class has too few samples
                logger.warning(
                    "Stratified split failed for surrogate model (error: %s), falling back to non-stratified split.",
                    ve,
                )
                X_train, X_test, y_train, y_test = train_test_split(
            ...
            unique_labels_y = sorted(y.unique())
            unique_labels_pred = sorted(pd.Series(y_pred_test).unique())
            present_labels = sorted(
                list(set(unique_labels_y).union(set(unique_labels_pred)))
            )
            class_map = {normal_label_val: "Normal", anomalous_label_val: "Anomalous"}
            target_names_report = [
                class_map.get(label, f"Class {label}") for label in present_labels
            ]
        ...
        importances = pd.Series(
            ...
            index=X.columns,
            name="surrogate_tree_feature_importance",
...
def generate_cluster_summary_text(
    ...
    if (
        cluster_mean_features_for_this_cluster is None
        or cluster_mean_features_for_this_cluster.empty
        or overall_mean_features is None
        or overall_mean_features.empty
    ):
        text += "Feature mean data not available for detailed characterization."
        return text
    common_features = cluster_mean_features_for_this_cluster.index.intersection(
        overall_mean_features.index
    )
    if not common_features.empty:
        ...
        relative_difference_pct = (
            (cluster_means_common - overall_means_common)
            / (overall_means_common.abs() + epsilon)
        ) * 100
        diff_df = pd.DataFrame(
            {
                "feature": common_features,
                "cluster_mean": cluster_means_common,
                "overall_mean": overall_means_common,
                "abs_rel_diff_pct": relative_difference_pct.abs(),
                "rel_diff_pct": relative_difference_pct,
            }
        )
        distinguishing_features_sorted = diff_df.sort_values(
            by="abs_rel_diff_pct", ascending=False
        )
        top_features_details = []
        for _, row in distinguishing_features_sorted.head(
            num_features_to_mention
        ).iterrows():
            if row["abs_rel_diff_pct"] > 20:
                direction = "higher" if row["rel_diff_pct"] > 0 else "lower"
                top_features_details.append(
                    f"{row['feature']} (value: {row['cluster_mean']:.2f}, notably {direction} than overall mean {row['overall_mean']:.2f})"
                )
            elif row["abs_rel_diff_pct"] > 5:
                direction = "higher" if row["rel_diff_pct"] > 0 else "lower"
                top_features_details.append(
                    f"{row['feature']} (value: {row['cluster_mean']:.2f}, slightly {direction} than overall mean {row['overall_mean']:.2f})"
                )
        if top_features_details:
            text += "It is characterized by: " + "; ".join(top_features_details) + "."
        else:
            text += "Its characteristics are broadly similar to average."
    else:
        text += "Could not compare cluster means."
    return text
...
def generate_anomaly_summary_text(
    device_id,
    anomaly_score,
    top_features_comparison: pd.DataFrame = None,
    surrogate_tree_importances: pd.Series = None,
    num_features_to_mention=3,
):
    """
    ...
    if surrogate_tree_importances is not None and not surrogate_tree_importances.empty:
        ...
        feature_list = [
            f"{idx} (importance: {val:.2f})"
            for idx, val in top_tree_features.items()
            if val > 0.01
        ]
        if feature_list:
            text += "; ".join(feature_list) + "."
        else:
            text += "surrogate tree did not highlight strong individual feature contributions."
        ...
        features_to_list = []
        for feature_name, row in top_features_comparison.head(
            num_features_to_mention
        ).iterrows():
            if row.get("Difference (Anomalous - Normal)", 0) > 0:
                "notably higher"
            else:
                "notably lower"
            features_to_list.append(
                f"{feature_name} (value: {row['Anomalous_Mean']:.2f} vs normal mean: {row['Normal_Mean']:.2f})"
            )
        if features_to_list:
            text += "; ".join(features_to_list) + "."
        else:
            text += "feature differences not strongly pronounced."
    else:
        text += "Detailed feature contribution analysis not available."
    return text
...
def analyze_event_correlations(
    ...
    if not isinstance(
        all_device_features_df_with_event_features, pd.DataFrame
    ) or not isinstance(result_labels, pd.Series):
        msg = "Inputs must be pandas DataFrame and Series."
        logger.warning(msg)
        return None, msg
    ...
    common_index = all_device_features_df_with_event_features.index.intersection(
        result_labels.index
    )
    ...
    event_count_cols = [
        col for col in df_aligned.columns if col.startswith(event_feature_prefix)
    ]
    ...
    try:
        ...
        event_analysis_df = pd.concat(
            [event_analysis_df, overall_mean_events.to_frame().T]
        )
...
def analyze_significant_event_types(
    ...
    if not event_count_cols:
        return (
            None,
            f"No event count features found with prefix '{event_feature_prefix}'.",
        )
    ...
    if baseline_label is not None:
        ...
        if not baseline_mask.any():
            return (
                None,
                "No devices found for the baseline group (all non at-risk devices).",
            )
    ...
    for event_col in event_count_cols:
        ...
        if incidence_baseline == 0:  # Avoid division by zero
            lift = (
                np.inf if incidence_at_risk > 0 else np.nan
            )  # Or some other indicator like -1 or a large number
        else:
        ...
        try:
            ...
            if (
                contingency_table.sum() > 0
                and np.all(contingency_table.sum(axis=0) > 0)
                and np.all(contingency_table.sum(axis=1) > 0)
            ):
                chi2, p, dof, expected = chi2_contingency(
                    contingency_table, correction=False
                )  # correction=False is often recommended
                p_value = p
                if (
                    np.any(expected < 5) and not chi2_note
                ):  # Check expected frequencies from chi2_contingency itself
                    chi2_note = "Fisher's exact test might be more appropriate due to low expected frequencies (<5)."
            else:
                chi2_note = (
                    "Skipped chi2: not enough data (e.g. all zeros in a row/column)."
                )
    ...
    results_df = results_df.sort_values(
        by=["lift", "p_value"], ascending=[False, True]
...
if __name__ == "__main__":
    ...
    features = pd.DataFrame(
        rng.rand(n_dev, n_feat),
        columns=[f"feat_{j}" for j in range(n_feat)],
        index=[f"dev_{i}" for i in range(n_dev)],
    )
    features.iloc[0:30, 0] += 2
    features.iloc[30:60, 0] -= 2
    features.iloc[0:20, 1] += 1.5
    mock_cluster_labels = pd.Series(
        [0] * (n_dev // 3) + [1] * (n_dev // 3) + [2] * (n_dev - 2 * (n_dev // 3)),
    ...
    importance_df, error = get_feature_importance_for_clusters_anova(
        features, mock_cluster_labels
    )
    if error:
        print(f"Error: {error}")
    else:
    ...
    comparison_df, error = compare_anomalous_vs_normal_features(
        features, mock_anomaly_labels
    )
    if error:
        print(f"Error: {error}")
    else:
    ...
    if summary_df is not None and importance_df is not None:
        ...
        print(
            generate_cluster_summary_text(
                0,
                (mock_cluster_labels == 0).sum(),
                len(features),
                summary_df.loc[0],
                overall_means_for_summary,
            )
        )
    if comparison_df is not None and not anomalous_indices.empty:
        print(
            generate_anomaly_summary_text(
                device_id=anomalous_indices[0],
                anomaly_score=-1.5,
                top_features_comparison=comparison_df,
            )
        )
    ...
    if not features.empty:
        ...
        if not features_cleaned_for_surrogate.empty:
            labels_for_surrogate = mock_anomaly_labels.loc[
                features_cleaned_for_surrogate.index
            ]
            if labels_for_surrogate.nunique() >= 2:
                (
                    ...
                    features_cleaned_for_surrogate,
                    labels_for_surrogate,
                    max_depth=3,
                    test_size=0.0,
                )
                if error_tree:
                    ...
                    print(
                        "Surrogate Tree Feature Importances:\n", tree_importances.head()
                    )
                    if tree_report:
                        print(
                            "Surrogate Tree Report:\n",
                            pd.DataFrame(tree_report).transpose(),
                        )
                    print("Surrogate tree model trained.")
            else:
                print("Skipping surrogate: Not enough distinct classes after cleaning.")
        else:
            print("Skipping surrogate: Features empty after NaN drop.")
    else:
        print("Skipping surrogate: 'features' not available.")

    print("\n--- Event Correlation Analysis ---")
    if "features" in locals() and "mock_anomaly_labels" in locals():
        mock_features_with_events = features.copy()
        mock_features_with_events["evt_count_Error_X"] = rng.randint(
            0, 5, size=len(features)
        )
        mock_features_with_events["evt_count_Warning_Y"] = rng.randint(
            0, 10, size=len(features)
        )
        mock_features_with_events["evt_count_Info_Z"] = rng.randint(
            0, 2, size=len(features)
        )  # Low count event
        anomalous_idx_for_event_test = mock_anomaly_labels[
            mock_anomaly_labels == -1
        ].index
        normal_idx_for_event_test = mock_anomaly_labels[mock_anomaly_labels == 1].index

        # Make Error_X more prevalent in anomalous
        mock_features_with_events.loc[
            anomalous_idx_for_event_test, "evt_count_Error_X"
        ] = rng.randint(1, 5, size=len(anomalous_idx_for_event_test))
        mock_features_with_events.loc[
            normal_idx_for_event_test, "evt_count_Error_X"
        ] = rng.randint(0, 2, size=len(normal_idx_for_event_test))
        # Make Info_Z more prevalent in normal (baseline)
        mock_features_with_events.loc[normal_idx_for_event_test, "evt_count_Info_Z"] = 1

        event_corr_df, error_ec = analyze_event_correlations(
            mock_features_with_events, mock_anomaly_labels
        )
        if error_ec:
            print(f"Error: {error_ec}")
        else:
            print(
                "Event Correlation with Anomaly Labels (Mean Event Counts):\n",
                event_corr_df,
            )

        if "mock_cluster_labels" in locals():
            event_corr_clusters_df, error_ecc = analyze_event_correlations(
                mock_features_with_events, mock_cluster_labels
            )
            if error_ecc:
                print(f"Error: {error_ecc}")
            else:
                print(
    ...
    if "mock_features_with_events" in locals() and "mock_anomaly_labels" in locals():
        ...
        if error_sig:
            print(
                f"Error in significant event analysis (anomalous vs all others): {error_sig}"
            )
        else:
        ...
        if error_sig_sb:
            print(
                f"Error in significant event analysis (anomalous vs specific baseline): {error_sig_sb}"
            )
        else:
            print(
        ...
        if "mock_cluster_labels" in locals() and mock_cluster_labels.nunique() > 1:
            ...
            if error_sig_c:
                print(
                    f"Error in significant event analysis (cluster 0 vs cluster 1): {error_sig_c}"
                )
            else:
                print(
            ...
            if error_sig_c_rest:
                print(
                    f"Error in significant event analysis (cluster 0 vs rest): {error_sig_c_rest}"
                )
            else:
                print(
