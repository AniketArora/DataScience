...
def extract_basic_stats(series: pd.Series, prefix=""):
    ...
    features[f"{prefix}std"] = (
        series_cleaned_for_stats.std()
    )  # Pandas handles single value series returning NaN
    ...
    features[f"{prefix}variance"] = (
        series_cleaned_for_stats.var()
    )  # Pandas handles single value series returning NaN
...
def extract_volatility_features(series: pd.Series, prefix=""):
    ...
    if diff_series.empty:  # Happens if series_cleaned has only 1 point after diff+dropna (i.e. original had 1 or 2 points)
        features[f"{prefix}mean_abs_diff"] = (
            0.0 if len(series_cleaned) == 1 else np.nan
        )  # Mean abs diff is 0 for 1 pt, undefined for 2 pts resulting in 1 diff
        features[f"{prefix}std_diff"] = (
            0.0 if len(series_cleaned) == 1 else np.nan
        )  # Std of diff is 0 for 1 pt, undefined for 2 pts
        return features
    features[f"{prefix}mean_abs_diff"] = diff_series.abs().mean()
    features[f"{prefix}std_diff"] = (
        diff_series.std()
    )  # Pandas std() returns NaN for a single value series (ddof=1)
    return features


def extract_autocorrelation_features(
    series: pd.Series, lags: list, prefix=""
):  # lags is now a list
    features = {}
    series_cleaned = series.dropna()
    ...
    if not valid_lags:
        for lag in lags:
            features[f"{prefix}acf_lag_{lag}"] = (
                np.nan
            )  # Ensure all original lag keys are present
        return features
    ...
    try:
        ...
        logger.warning(
            f"ACF calculation failed for series {series.name if series.name else 'Unnamed'}: {e}"
        )
        for lag in lags:
            features[f"{prefix}acf_lag_{lag}"] = np.nan
    return features
...
def extract_rolling_stats_features(series: pd.Series, windows: list, prefix=""):
    ...
    for window in windows:
        ...
        features[f"{prefix}std_of_means_w{window}"] = (
            std_of_means if pd.notna(std_of_means) else 0.0
        )
        ...
        if window == 1:
            ...
            rolling_std = (
                series_cleaned.rolling(window=window, min_periods=2).std().fillna(0)
            )
        ...
        features[f"{prefix}std_of_stds_w{window}"] = (
            std_of_stds if pd.notna(std_of_stds) else 0.0
        )
...
def extract_event_features_for_series(
    ...
    if (
        not isinstance(target_series, pd.Series)
        or not isinstance(target_series.index, pd.DatetimeIndex)
        or target_series.empty
    ):
        return (
            features,
            "Target series is invalid or empty for event feature extraction.",
        )
    ...
    if not all(col in device_event_df.columns for col in required_cols):
        return (
            features,
            f"Missing essential event columns in device_event_df. Expected: {', '.join(required_cols)}.",
        )

    try:
        events_df_processed = device_event_df.copy()
        if not pd.api.types.is_datetime64_any_dtype(
            events_df_processed[event_timestamp_col_name]
        ):
            events_df_processed[event_timestamp_col_name] = pd.to_datetime(
                events_df_processed[event_timestamp_col_name], errors="coerce"
            )

        events_df_processed = events_df_processed.dropna(
            subset=[event_timestamp_col_name, event_type_col_name]
        )
    ...
    if all_possible_event_types_cleaned_names:
        ...
        event_types_to_process_locally = event_type_counts.iloc[
            : min(top_n_event_types, len(event_type_counts))
        ].index.tolist()

    if not event_types_to_process_locally:
        return features, None

    relevant_events_sorted = relevant_events.sort_values(
        by=event_timestamp_col_name, ascending=False
    )
    series_duration_hours = (
        series_end_time - series_start_time
    ...
    for raw_event_type in event_types_to_process_locally:
        ...
        type_specific_events = relevant_events_sorted[
            relevant_events_sorted[event_type_col_name] == raw_event_type
        ]
        ...
        if series_duration_hours > 1e-6:
            features[f"{prefix}freq_per_hour_{clean_event_type_name}"] = (
                current_count / series_duration_hours
            )
        else:
...
def generate_all_features_for_series(
    ...
    try:
        ...
        acf_lags_to_use = (
            acf_lags_list
            if acf_lags_list is not None and len(acf_lags_list) > 0
            else [1, 5, 10]
        )
        default_rolling_windows = [1, 5, 10, 20]
        rolling_windows_to_use = (
            rolling_windows_list
            if rolling_windows_list is not None and len(rolling_windows_list) > 0
            else default_rolling_windows
        )
        ...
        all_features.update(
            extract_autocorrelation_features(
                series, lags=acf_lags_to_use, prefix=f"{name}acf_"
            )
        )
        ...
        if rolling_windows_to_use:  # Ensure list is not empty
            all_features.update(
                extract_rolling_stats_features(
                    series, windows=rolling_windows_to_use, prefix=f"{name}roll_"
                )
            )
        else:  # If rolling_windows_to_use ended up empty, ensure no error and potentially log
            logger.debug(
                f"No rolling windows to process for series {series_name_for_log}."
            )
        ...
        if all_possible_event_types:
            for clean_event_type_name in all_possible_event_types:
                temp_event_features_initialized[
                    f"{name}evt_count_{clean_event_type_name}"
                ] = 0
                temp_event_features_initialized[
                    f"{name}evt_hours_since_last_{clean_event_type_name}"
                ] = np.nan
                temp_event_features_initialized[
                    f"{name}evt_freq_per_hour_{clean_event_type_name}"
                ] = 0.0
        all_features.update(temp_event_features_initialized)

        event_features_calculated_dict, event_err_msg = None, None
        if (
            device_event_df is not None
            and not device_event_df.empty
            and isinstance(series.index, pd.DatetimeIndex)
            and not series.empty
        ):
            event_features_calculated_dict, event_err_msg = (
            ...
            if event_err_msg:
                logger.warning(
                    "Error during event feature extraction for series %s: %s",
                    series_name_for_log,
                    event_err_msg,
                )
...
if __name__ == "__main__":
    ...
    data = [
        i + (i % 7) * 2 + np.random.randn() * 5 + (i / 10) ** 1.5 for i in range(50)
    ]
    ...
    sample_event_data = {
        "timestamp": pd.to_datetime(
            ["2023-01-05", "2023-01-10", "2023-01-05", "2023-02-10", "2023-01-15"]
        ),
        "event_type": [
            "Error_A",
            "Warning_B",
    ...
    ts_with_event_features_consistent, err = generate_all_features_for_series(
        ...
        acf_lags_list=[2, 4],
        rolling_windows_list=[2, 4, 8],  # Test with new params
        top_n_event_types_to_focus=3,
    )
    if err:
        print(f"Error: {err}")
    elif ts_with_event_features_consistent is not None:
        print(
            ts_with_event_features_consistent[
                ts_with_event_features_consistent.index.str.startswith("sensor_X_evt_")
            ]
        )

    print("\n--- Event Features for series with no relevant events ---")
    short_ts_for_event_test = pd.Series(
        np.arange(5), index=pd.date_range(start="2020-01-01", periods=5, freq="D")
    )
    event_features_none, err = generate_all_features_for_series(
        short_ts_for_event_test,
        name="short_evt_test_",
        device_event_df=sample_device_events_df,
        all_possible_event_types=global_cleaned_event_types,
    )
    if err:
        print(f"Error: {err}")
    elif event_features_none is not None:
        print(
            event_features_none[
                event_features_none.index.str.startswith("short_evt_test_evt_")
            ]
        )
    ...
    target_series_for_complex_events = pd.Series(
        np.arange(10),
    ...
    if err:
        print(f"Error: {err}")
    elif event_features_complex is not None:
        print(
            event_features_complex[
                event_features_complex.index.str.startswith("complex_evt_")
            ]
        )
...
def run_feature_engineering_for_all_devices(
    ...
    if (
        data_df_original_serializable.empty
        or ts_specs_serializable.get("timestamp_col", "None") == "None"
        or ts_specs_serializable.get("selected_value_col_for_analysis", "None")
        == "None"
    ):
        logger.warning(
            "Aborting feature engineering for all devices due to empty data or missing critical spec columns."
        )
        return pd.DataFrame(), []
    ...
    val_col_to_process_for_all = ts_specs_serializable[
        "selected_value_col_for_analysis"
    ]

    temp_id_col_name_all_features = "_temp_unique_id_all_features_"
    if id_cols_for_all:
        df_for_all_features[temp_id_col_name_all_features] = (
            df_for_all_features[id_cols_for_all].astype(str).agg("-".join, axis=1)
        )
        unique_entities = sorted(
            df_for_all_features[temp_id_col_name_all_features].unique()
        )
    else:
    ...
    event_id_col_main_all = ts_specs_serializable.get(
        "event_device_id_col", "device_id"
    )
    event_type_col_main_all = ts_specs_serializable.get(
    ...
    rolling_windows_config = ts_specs_serializable.get(
    ...
    for i, entity_id_val in enumerate(unique_entities):
        entity_df_all = df_for_all_features[
            df_for_all_features[temp_id_col_name_all_features] == entity_id_val
        ]
        try:
            entity_df_all[ts_col_for_all] = pd.to_datetime(
                entity_df_all[ts_col_for_all], errors="coerce"
            )
            entity_df_all = entity_df_all.dropna(
                subset=[ts_col_for_all, val_col_to_process_for_all]
            )
            ...
            if entity_df_all.empty:
                error_summary_list.append(
                    (
                        entity_id_val,
                        "No valid data after NA drop for value or timestamp column.",
                    )
                )
                continue
            ...
            processed_entity_series_all = (
                entity_df_all.groupby(ts_col_for_all)[val_col_to_process_for_all]
                .mean()
                .rename(val_col_to_process_for_all)
            )

            if (
                processed_entity_series_all.empty
                or len(processed_entity_series_all) < 2
            ):
                error_summary_list.append(
                    (
                        entity_id_val,
                        f"Series too short after processing (length {len(processed_entity_series_all)}).",
                    )
                )
                continue
            ...
            if (
                not event_df_main_for_all.empty
                and id_cols_for_all
                and entity_id_val != "DefaultTimeSeries"
            ):
                if event_id_col_main_all in event_df_main_for_all.columns:
                    device_specific_events_all = event_df_main_for_all[
                        event_df_main_for_all[event_id_col_main_all].astype(str)
                        == str(entity_id_val)
                    ]
            elif (
                not event_df_main_for_all.empty
                and not id_cols_for_all
                and entity_id_val == "DefaultTimeSeries"
            ):
                device_specific_events_all = event_df_main_for_all
            ...
            features_series, error_msg_feat = generate_all_features_for_series(
                ...
                device_event_df=device_specific_events_all
                if not device_specific_events_all.empty
                else None,
            ...
            if error_msg_feat:
                ...
                logger.warning(
                    "Feature generation error for entity '%s': %s",
                    entity_id_val,
                    error_msg_feat,
                )
                continue
            if features_series is not None and not features_series.empty:
                ...
                error_summary_list.append(
                    (entity_id_val, "No features generated, skipping.")
                )
                logger.info(
                    "No features generated for entity '%s' (series might be too short or unsuitable, but no hard error), skipping.",
                    entity_id_val,
                )
            ...
            logger.error(
                "Unhandled exception processing entity '%s' in background: %s",
                entity_id_val,
                e_feat_loop,
                exc_info=True,
            )
    ...
    logger.info(
        "Completed feature engineering for %d devices. %d errors/warnings.",
        len(all_features_list),
        len(error_summary_list),
    )
