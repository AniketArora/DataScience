...
def test_decompose_time_series_additive(sample_series_decomposition):
    result, error = decompose_time_series(
        sample_series_decomposition, model="additive", period=7
    )
...
def test_decompose_time_series_multiplicative(sample_series_decomposition):
    positive_series = (
        sample_series_decomposition + abs(sample_series_decomposition.min()) + 1
    )
    result, error = decompose_time_series(
        positive_series, model="multiplicative", period=7
    )
    assert error is None
    assert isinstance(result, DecomposeResult)
...
def test_decompose_time_series_general_exception(
    mock_seasonal_decompose, sample_series_decomposition
):
