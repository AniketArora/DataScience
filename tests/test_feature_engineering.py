...
import pandas as pd
import numpy as np
from unittest.mock import patch  # Import call for checking arguments
...
def normal_series():
    ...
    data = [
        i + (i % 7) * 2 + np.random.randn() * 2 + (i / 10) ** 1.5 for i in range(50)
    ]
...
def test_extract_autocorrelation_features_normal(normal_series):
    features = extract_autocorrelation_features(
        normal_series, lags=[1, 2, 5], prefix="test_"
    )
...
def test_generate_all_features_uses_custom_acf_lags(mock_extract_acf, normal_series):
    ...
    mock_return_data = {
        f"ts_acf_acf_lag_{lag}": np.random.rand() for lag in custom_lags
    }
    mock_extract_acf.return_value = pd.Series(mock_return_data)

    features_series, _ = generate_all_features_for_series(
        normal_series, acf_lags_list=custom_lags
    )
    mock_extract_acf.assert_called_with(
        normal_series, lags=custom_lags, prefix="ts_acf_"
    )
...
def test_generate_all_features_uses_default_acf_lags(mock_extract_acf, normal_series):
    ...
    mock_return_data = {
        f"ts_acf_acf_lag_{lag}": np.random.rand() for lag in default_lags
    }
    mock_extract_acf.return_value = pd.Series(mock_return_data)

    features_series, _ = generate_all_features_for_series(
        normal_series, acf_lags_list=None
    )
    mock_extract_acf.assert_called_with(
        normal_series, lags=default_lags, prefix="ts_acf_"
    )
...
def test_generate_all_features_uses_custom_rolling_windows(
    mock_extract_rolling, normal_series
):
    ...
    features_series, _ = generate_all_features_for_series(
        normal_series, rolling_windows_list=custom_windows
    )
    mock_extract_rolling.assert_called_with(
        normal_series, windows=custom_windows, prefix="ts_roll_"
    )
...
@patch("src.analysis_modules.feature_engineering.extract_rolling_stats_features")
def test_generate_all_features_uses_default_rolling_windows(
    mock_extract_rolling, normal_series
):
    ...
    features_series, _ = generate_all_features_for_series(
        normal_series, rolling_windows_list=None
    )
    mock_extract_rolling.assert_called_with(
        normal_series, windows=default_windows, prefix="ts_roll_"
    )
    # Optionally, assert that features from the mock are present
    for w in default_windows:
        assert f"ts_roll_mean_of_means_w{w}" in features_series.index
...
def test_rolling_stats_empty_series_input(empty_series):
    features = extract_rolling_stats_features(
        empty_series, windows=[1, 5], prefix="test_"
    )
...
def test_rolling_stats_all_nan_series_input(all_nan_series):
    features = extract_rolling_stats_features(
        all_nan_series, windows=[1, 5], prefix="test_"
    )
    assert pd.isna(features["test_mean_of_means_w1"])
    assert pd.isna(features["test_mean_of_stds_w5"])
...
def test_rolling_stats_window_larger_than_series(short_series):  # len 8
    ...
    features = extract_rolling_stats_features(
        short_series, windows=[L + 1, L + 5], prefix="test_"
    )
...
def test_rolling_stats_constant_series(constant_series):  # len 20, value 5.0
    features = extract_rolling_stats_features(
        constant_series, windows=[2, 5], prefix="const_"
    )
    for w in [2, 5]:
...
def test_rolling_stats_short_series_for_std_min_periods(very_short_series):  # len 3
    ...
    features = extract_rolling_stats_features(
        very_short_series, windows=[3], prefix="short3_"
    )
    assert not pd.isna(features["short3_mean_of_means_w3"])
    assert not pd.isna(
        features["short3_std_of_means_w3"]
    )  # std of (mean of [0,1,2]) is 0
    ...
    features_w2 = extract_rolling_stats_features(
        very_short_series, windows=[2], prefix="short2_"
    )
...
def test_basic_stats_two_value_series(two_value_series):  # Series([10.0, 12.0])
    ...
    assert pd.notna(
        features["kurtosis_val"]
    )  # Scipy kurtosis for 2 values is -2 (or NaN if fisher=False)
...
def sample_data_for_orchestration():
    ...
    ts_specs = {
        ...
        "acf_lags": [1, 2],
        "rolling_windows": [2, 3],  # Add new configurable params
    }
    event_data = {
        "device_id": ["dev1", "dev1", "dev2"],
        "event_timestamp": pd.to_datetime(
            ["2023-01-01 00:00:30", "2023-01-01 00:01:30", "2023-01-01 00:00:30"]
        ),
...
def test_run_all_devices_success_with_configs(
    mock_generate_features, sample_data_for_orchestration
):
...
@patch("src.analysis_modules.feature_engineering.generate_all_features_for_series")
def test_run_all_devices_generate_features_returns_error_orchestration(
    mock_generate_features, sample_data_for_orchestration
):
    data_df, ts_specs, event_df, global_events = sample_data_for_orchestration
    ...
    def side_effect_func(
        ...
        name,
        device_event_df,
        all_possible_event_types,
        event_type_col,
        event_ts_col,
        top_n_event_types_to_focus,
        acf_lags_list,
    ...
    assert (
        len(error_list) == 2
    )  # This will fail if the "No features generated" case isn't added
    assert any(
        item[0] == "dev2" and "Simulated processing error for dev2" in item[1]
        for item in error_list
    )
    assert any(
        item[0] == "dev3" and "No features generated, skipping." in item[1]
        for item in error_list
    )
...
def test_generate_all_features_normal_rolling(normal_series):
    features, _ = generate_all_features_for_series(
        normal_series, name="ts_", rolling_windows_list=[5, 10]
    )
    assert "ts_roll_mean_of_means_w5" in features and not pd.isna(
        features["ts_roll_mean_of_means_w5"]
    )
    assert "ts_roll_mean_of_stds_w5" in features and not pd.isna(
        features["ts_roll_mean_of_stds_w5"]
    )


def test_generate_all_features_short_series_rolling(short_series):  # len 8
    features, _ = generate_all_features_for_series(
        short_series, name="ts_", rolling_windows_list=[1, 4, 8, 10]
    )
    assert "ts_roll_mean_of_means_w4" in features and not pd.isna(
        features["ts_roll_mean_of_means_w4"]
    )
    assert "ts_roll_mean_of_means_w1" in features and not pd.isna(
        features["ts_roll_mean_of_means_w1"]
    )
    # Window of 8 on series of length 8 should be fine
    assert "ts_roll_mean_of_means_w8" in features and not pd.isna(
        features["ts_roll_mean_of_means_w8"]
    )
    # Window of 10 on series of length 8 should also produce a result with min_periods=1
    assert "ts_roll_mean_of_means_w10" in features and not pd.isna(
        features["ts_roll_mean_of_means_w10"]
    )


def test_generate_all_features_constant_series_rolling(constant_series):
    features, _ = generate_all_features_for_series(
        constant_series, name="ts_", rolling_windows_list=[5]
    )
