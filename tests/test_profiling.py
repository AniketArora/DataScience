...
def test_get_missing_values_summary(sample_series_profiling):
    ...
    assert (
        summary[summary["Metric"] == "Missing Percentage (%)"]["Value"].iloc[0]
        == "20.00%"
    )


def test_perform_stationarity_test_non_stationary(sample_series_profiling):
    results = perform_stationarity_test(
        sample_series_profiling.dropna()
    )  # dropna() here because ADF itself drops, but this sample is non-stationary even then
...
def test_perform_stationarity_test_on_random_noise():  # Renamed and uses pure noise
    ...
    assert results.get(
        "Is Stationary (at chosen alpha)"
    ), f"Expected stationary for random noise, p-value: {results.get('p-value')}"
...
def test_perform_stationarity_test_on_trend_series():  # New test for clear non-stationarity
    ...
    assert not results.get(
        "Is Stationary (at chosen alpha)"
    ), f"Expected non-stationary for trend series, p-value: {results.get('p-value')}"
...
def test_perform_stationarity_test_all_nan_series():
    ...
    assert (
        results["error"]
        == "Series is empty after dropping NaN values. Cannot perform ADF test."
    )
