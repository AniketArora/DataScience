...
def test_detect_anomalies_zscore_global(sample_series_anomalies):
    anomalies, z_scores, error = detect_anomalies_zscore(
        sample_series_anomalies, threshold=2.0
    )
...
def test_detect_anomalies_zscore_rolling(sample_series_anomalies):
    anomalies, z_scores, error = detect_anomalies_zscore(
        sample_series_anomalies, threshold=2.0, window=5
    )
...
def test_detect_anomalies_iqr(sample_series_anomalies):
    anomalies, bounds_info, error = detect_anomalies_iqr(
        sample_series_anomalies, multiplier=1.5
    )
    assert error is None
    assert anomalies is not None
...
def test_iqr_all_nan_series():
    anomalies, bounds, error = detect_anomalies_iqr(
...
def test_detect_anomalies_isolation_forest_valid(sample_feature_df):
    labels, scores, error = detect_anomalies_isolation_forest(
...
class TestAnomalyDetectionAnalysisModule:
    ...
    def test_run_analysis_iforest(self, module, sample_feature_df):
        params = {"selected_method": "Isolation Forest", "iforest_contamination": 0.1}
