...
def test_gather_settings_full(mock_st_session_state):
    with patch('src.config_utils.st.session_state', mock_st_session_state):
        mock_st_session_state.time_series_specs = {
            'id_cols': ['device_id'], 'timestamp_col': 'time_col', 'value_cols': ['val1', 'val2'],
            'selected_id': 'dev1', 'selected_value_col_for_analysis': 'val1'
        }
        mock_st_session_state.if_contam_general = 0.05
        mock_st_session_state.ocsvm_nu_general = 0.02
        mock_st_session_state.clustering_module_params = {'method': 'K-Means', 'n_clusters': 3}
...
def test_apply_settings_full(mock_st_session_state):
    with patch('src.config_utils.st.session_state', mock_st_session_state):
        sample_settings = {
            ...
            "feature_engineering_settings": {"acf_lags": [5, 10], "rolling_windows": [3, 6]}
        }
...
def test_apply_settings_handles_none_values(mock_st_session_state):
    with patch('src.config_utils.st.session_state', mock_st_session_state):
        ...
        settings_with_none = {"anomaly_detection_settings": {"IsolationForest": {"contamination": None}}}
...
def test_apply_settings_error_handling(mock_st_session_state):
    with patch('src.config_utils.st.session_state', mock_st_session_state):
        ...
        with patch('src.config_utils._set_nested_session_state', side_effect=Exception("mock error")):
