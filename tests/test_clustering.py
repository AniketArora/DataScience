...
def test_perform_kmeans_clustering_basic(sample_feature_df):
    ...
    n_clusters = 3
    labels, model, error = perform_kmeans_clustering(
        sample_feature_df, n_clusters=n_clusters, random_state=42
    )
...
def test_perform_kmeans_clustering_scale_data(sample_feature_df):
    ...
    n_clusters = 3
    # Test with scaling
    labels_scaled, _, error_scaled = perform_kmeans_clustering(
        sample_feature_df, n_clusters=n_clusters, scale_data=True, random_state=42
    )
    assert error_scaled is None
    assert labels_scaled is not None
    assert (
        len(np.unique(labels_scaled)) <= n_clusters
    )  # Scaling might change exact number of clusters found if some become empty

    # Test without scaling
    labels_unscaled, _, error_unscaled = perform_kmeans_clustering(
        sample_feature_df, n_clusters=n_clusters, scale_data=False, random_state=42
    )
...
def test_perform_kmeans_clustering_edge_cases(sample_feature_df):
    ...
    # n_clusters too high
    labels, model, error = perform_kmeans_clustering(
        sample_feature_df, n_clusters=len(sample_feature_df) + 1
    )
    assert (
        f"Number of clusters ({len(sample_feature_df) + 1}) cannot exceed number of samples ({len(sample_feature_df)})"
        in error
    )
    assert labels is None
    assert model is None
...
def test_get_kmeans_elbow_silhouette_data(sample_feature_df):
    ...
    if max_k_test < 2:
        pytest.skip(
            "Not enough samples for k-means stats range test after considering max_k."
        )

    k_range = range(2, max_k_test + 1)

    results_df, error = get_kmeans_elbow_silhouette_data(
        sample_feature_df, k_range=k_range, scale_data=True
    )
    assert error is None
...
def test_perform_dbscan_clustering_basic(sample_feature_df):
    ...
    labels, model, error = perform_dbscan_clustering(
        sample_feature_df, eps=1.0, min_samples=3, scale_data=True
    )
    assert error is None
    assert labels is not None
    assert model is not None
...
class TestClusteringAnalysisModule:
    ...
    def test_run_analysis_kmeans(self, module, sample_feature_df):
        params = {"method": "K-Means", "n_clusters": 3, "scale_data_kmeans": True}
