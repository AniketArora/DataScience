...
def test_handle_postgres_connection(mock_st):
    with patch("src.app_logic.st", mock_st):
        ...
        with patch("src.app_logic.get_schemas_postgres") as mock_get_schemas, patch(
            "src.app_logic.get_tables_postgres"
        ) as mock_get_tables, patch(
            "src.app_logic.fetch_data_postgres"
        ) as mock_fetch_data:
...
def test_handle_time_series_settings(mock_st):
    with patch("src.app_logic.st", mock_st):
        ...
        assert mock_st.session_state.time_series_specs["value_cols"] == [
            "value1",
            "value2",
        ]
...
def test_handle_population_analysis_settings(mock_st):
    with patch("src.app_logic.st", mock_st):
        ...
        with patch(
            "src.app_logic.concurrent.futures.ProcessPoolExecutor"
        ) as mock_executor:
            handle_population_analysis_settings()
...
def test_handle_save_load_app_state_save(mock_st):
    with patch("src.app_logic.st", mock_st):
        with patch("src.app_logic.gather_settings_for_save") as mock_gather, patch(
            "src.app_logic.json.dumps"
        ) as mock_dumps:
            mock_st.sidebar.button.return_value = True
...
def test_handle_save_load_app_state_load(mock_st):
    with patch("src.app_logic.st", mock_st):
        ...
        with patch("src.app_logic.json.load") as mock_load, patch(
            "src.app_logic.apply_loaded_settings_to_session_state"
        ) as mock_apply:
